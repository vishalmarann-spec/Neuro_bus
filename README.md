...
Planning baseline is locked. Milestone 1 provides trustworthy evidence storage. Milestone 2 adds a provider-independent extraction contract, schema and provenance validation, entities, mentions, claims, evidence links, UEO retrieval, model audit records, invalid-output quarantine, idempotent reruns, and append-only analyst review decisions. Milestone 3 now provides conservative entity resolution, deterministic claim clustering, duplicate-aware corroboration and contradiction scoring, explicit component explanations, and disputed-cluster retrieval. The checked-in model benchmark is synthetic infrastructure validation only; real source connectors, a curated evaluation set, production-model selection, and upstream citation-family detection remain open work.
